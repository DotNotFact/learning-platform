...
export function EditCourseDialog({
  ...
  async function onSubmit(values: z.infer<typeof courseSchema>) {
    try {
      setLoading(true)
      const response = await fetch(`/api/courses/${course.courseUid}`, {
        method: "PATCH",
      ...
      if (!response.ok) {
        showToast(data.error || "Ошибка при обновлении курса", "error")
        return
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>Измените информацию о курсе</DialogDescription>
        </DialogHeader>
        <Form {...form}>
